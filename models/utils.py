...
import math
from DCN.modules.deform_conv2d import DeformConv2d
...
class DropBlock(nn.Module):
    ...
    def calculate_gamma(self, x):
        ...
        return  (1-self.keep_prob) * x.shape[-1]**2/(self.block_size**2 * (x.shape[-1] - self.block_size + 1)**2) 
    ...
    def __init__(self, in_ch, out_ch, n_anchors, rfb=False, sep=False):
        ...
        if sep:
            ...
            self.conv_offset = nn.Conv2d(
                in_channels=2*n_anchors, 
                out_channels=2*9*n_anchors, 
                groups=n_anchors, 
                kernel_size=1,
                stride=1,
                padding=0
